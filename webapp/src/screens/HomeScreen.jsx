...
const workflowSteps = [
  { num: '01', title: 'Spark a thought', desc: '3am thoughts, reflections from the day, ideas that hit you while talking to friends - see what matters to everyone on OpenBrain', icon: '💡' },
  { num: '02', title: 'Your personal knowledge assistant', desc: 'Talk or write - whichever you prefer, ', descSuffix: ' organizes your thoughts instantly, so you can find them later when it matters.', icon: '🔍' },
  { num: '03', title: 'Save what matters', desc: 'Move your favorite ideas into ', descSuffix: ' for your own exploration.', icon: '📥' },
  { num: '04', title: 'Build over time', desc: 'With OpenBrain, turn saved ideas into durable knowledge for future projects.', icon: '🗂️' },
];

const openBrainLiveFeedFeatureTooltips = [
  { id: 'markdown', label: 'Markdown rendering', marker: 'M', text: 'Card display supports full Markdown, so thoughts render beautifully with structure and emphasis.' },
  { id: 'reactions', label: 'Reactions', marker: 'R', text: 'Readers can react in one tap, helping you see how others relate to your thoughts.' },
  { id: 'save', label: 'Save to SecondBrain', marker: 'S', text: 'Thoughts can be saved into SecondBrain for self-reflection and personal reference.' },
];

const secondBrainArchiveFeatureTooltips = [
  { id: 'capture', label: 'Capture everything', marker: 'C', text: 'Archive thoughts, ideas, TODOs and notes in one place so nothing important gets lost.\n\n Supports LLM conversation export uploads as well, so your AI interactions can also be part of your knowledge base.' },
  { id: 'markdown', label: 'Markdown support', marker: 'M', text: 'Entries preserve full Markdown formatting for clean, readable long-form notes.' },
  { id: 'searchable', label: 'Searchable forever', marker: 'S', text: 'Your archive stays structured and searchable, so key context is easy to retrieve later.' },
];

function Logo({ variant = 'second' }) {
  if (variant === 'open') {
    return <span className="hs-logo">open<span className="hs-logo-accent hs-open">brain</span></span>;
  }
  return <span className="hs-logo">second<span className="hs-logo-accent hs-second">brain</span></span>;
}

function ProductCard({ number, tagline, description, features, tone, buttonLabel }) {
  return (
    <section className="hs-product-card">
      <p className="hs-product-number">{number}</p>
      <h3 className="hs-product-title"><Logo variant={tone === 'open' ? 'open' : 'second'} /></h3>
      ...
      <button type="button" className={`hs-action-btn ${tone}`}>{buttonLabel}</button>
    </section>
  );
}
...
export default function HomeScreen() {
  ...
  const activeOpenBrainTooltip = openBrainLiveFeedFeatureTooltips.find((item) => item.id === activeOpenBrainTooltipId) || openBrainLiveFeedFeatureTooltips[0];
  const activeSecondBrainTooltip = secondBrainArchiveFeatureTooltips.find((item) => item.id === activeSecondBrainTooltipId) || secondBrainArchiveFeatureTooltips[0];
  const themeVars = {
  ...
  return (
    <main className="hs-page" style={themeVars} data-sticky-header-indices={stickyHeaderIndices.join(',')}>
      <section className="hs-sticky-section hs-hero hs-hero-quote">
        <h1>If you could save just one thought before the day ends, what would it be?</h1>
      </section>

      <section className="hs-sticky-section hs-hero hs-hero-video">
        <video autoPlay muted loop playsInline preload="auto" className="hs-bg-video" src="/ebony-forsyth-dupe.mp4" />
      ...
      <section className="hs-sticky-section hs-hero hs-products-hero">
        <article>
          <Logo variant="second" />
          <p>Your personal knowledge OS: capture notes, thoughts, todos, reminders, and even imported LLM conversations in one place. From quick brain-dumps to structured memory, automatically organize scattered inputs with SecondBrain.</p>
          <button type="button" className="hs-action-btn second">Try SecondBrain</button>
        </article>
        <article>
          <Logo variant="open" />
          <p>What if we only had one thought a day to share with the world? See the foremost thought on everyone&apos;s minds on the OpenBrain feed. When something resonates, save it straight into SecondBrain so inspiration turns into usable knowledge.</p>
          <button type="button" className="hs-action-btn open">Try Openbrain</button>
        </article>
      </section>
      ...
      <section className="hs-sticky-section hs-preview-panel hs-open-panel">
        ...
        <div className="hs-preview-card">
          ...
          <p className="hs-preview-thought">Could onboarding ask for a single user outcome first, then tailor setup around that?</p>
          <p className="hs-preview-tags">#product #ux #onboarding</p>
          <p className="hs-preview-meta">felt_this 3 · me_too 1 · made_me_think 2 · saves 5</p>
        </div>
      ...
      <section className="hs-sticky-section hs-preview-panel hs-second-panel">
        ...
        <img src="/secondbrain-transparent-v2.png" alt="SecondBrain preview" className="hs-second-preview" />
        <div className="hs-hotspots">
        ...
        <div className="hs-workflow-grid">
          {workflowSteps.map((step) => (
            <article key={step.num} className="hs-workflow-card">
              ...
              <p>
                {step.desc}
                {step.descSuffix ? <span className="hs-step-inline-logo"><Logo variant="second" /></span> : null}
                {step.descSuffix || ''}
              </p>
            </article>
          ))}
      ...
      <section className="hs-sticky-section hs-cta-row">
        <article className="hs-cta-card open">
          ...
          <p>Join people sharing their foremost ponderings every day on <Logo variant="open" />.</p>
          <button type="button" className="hs-action-btn open">Try OpenBrain</button>
        </article>
        <article className="hs-cta-card second">
          <h3>Build the brain you wish you had</h3>
          <p>Your notes, your insights, your decisions in one searchable archive with <Logo variant="second" />.</p>
          <button type="button" className="hs-action-btn second">Try SecondBrain</button>
        </article>
      </section>
      ...
      <footer className="hs-footer">
        <p>
          © {currentYear} <Logo variant="open" /> &amp; <Logo variant="second" />
        </p>
      </footer>
    </main>
  );
