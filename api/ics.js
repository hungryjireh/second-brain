...
export default async function handler(req, res) {
  ...
  if (!userId)
    return res
      .status(401)
      .json({ error: "invalid auth token payload: expected UUID user id" });
  ...
  try {
    ...
    if (entry.category !== "reminder" || !entry.remind_at) {
      return res
        .status(400)
        .json({ error: "entry is not a reminder with schedule" });
    }

    const ics = buildReminderIcs(entry);
    const reminderName = toSafeFilePart(
      entry.title || entry.content || "reminder",
    );
