...
test.after(() => {
  ...
  process.env.TELEGRAM_TOKEN_ENCRYPTION_KEY =
    originalEnv.TELEGRAM_TOKEN_ENCRYPTION_KEY;
});
...
test("telegram link token is stored encrypted and returned decrypted via RPC lookup", async () => {
  ...
  global.fetch = async (input, init = {}) => {
    ...
    if (
      path === "/rest/v1/rpc/lookup_telegram_link_by_chat_id" &&
      method === "POST"
    ) {
      const row = rows.get(String(body?.p_chat_id));
      const payload = row
        ? [{ user_id: row.user_id, auth_token: row.auth_token }]
        : [];
...
test("telegram RPC lookup still supports legacy plaintext auth_token rows", async () => {
  ...
  assert.deepEqual(linked, {
    userId: "legacy-user",
    authToken: "legacy-plaintext-token",
  });
});
...
test("telegram link write can store one token while authorizing with another token", async () => {
  ...
  assert.equal(
    lastAuthorizationHeader,
    "Bearer short-lived-supabase-token-for-write",
  );
});
