...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const rows = await supabaseRequest('/rest/v1/profiles', {
        ...
        query: usingUsernameLookup
          ? { select: 'id,username,bio,avatar_url,streak_count,last_posted_at,timezone,username_changed_once', username: `eq.${requestedUsername}`, limit: 1 }
          : { select: 'id,username,bio,avatar_url,streak_count,last_posted_at,timezone,username_changed_once', id: `eq.${targetId}`, limit: 1 },
        authToken,
      });

      if (!rows?.[0]) return json(res, 404, { profile: null });
      const profile = rows[0];
      const usernameChangedOnce = assertBooleanField(profile.username_changed_once, 'profiles.username_changed_once');
    ...
    if (req.method === 'POST') {
      ...
      assertBooleanField(profile.username_changed_once, 'profiles.username_changed_once');
    ...
    const usernameChangedOnce = assertBooleanField(existingProfile.username_changed_once, 'profiles.username_changed_once');
    ...
    if (requestedUsername && requestedUsername !== String(existingProfile.username || '')) {
