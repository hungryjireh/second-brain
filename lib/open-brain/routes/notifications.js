...
export default async function handler(req, res) {
    ...
    try {
      ...
      const usernameById = new Map(
        (Array.isArray(profileRows) ? profileRows : [])
          .map(profile => [profile?.id, profile?.username || null])
      );
      const thoughtById = new Map(
        (Array.isArray(thoughtRows) ? thoughtRows : [])
          .map(thought => [thought?.id, { id: thought?.id, share_slug: thought?.share_slug || null }])
      );
      ...
      const notificationsWithActorProfiles = notifications.map(item => ({
        ...
        thought: thoughtById.get(parseNotificationPayload(item?.payload)?.thought_id) || null,
      }));
  ...
  const thoughtId = String(payload?.thought_id || req.body?.thought_id || '').trim();
  const reactionType = String(payload?.reaction_type || req.body?.reaction_type || '').trim();
  ...
  if (type === 'reaction') {
    if (!isUuid(thoughtId)) return json(res, 400, { error: 'thought_id must be a valid UUID' });
    if (!REACTION_TYPES.has(reactionType)) return json(res, 400, { error: 'unsupported reaction type' });
  }

  try {
    const insertRow = type === 'reaction'
      ? {
          user_id: userId,
          actor_id: actorId,
          type,
    ...
    return json(res, 201, {
      notification: {
        ...
        actor_id: actorId,
        type,
        payload: type === 'reaction' ? { thought_id: thoughtId, reaction_type: reactionType } : {},
      },
    });
  } catch (err) {
    const message = err?.data?.message || err.message || 'request failed';
