...
function computeNextStreak({ streakCount, lastPostedAt, timezone }) {
  ...
  const previousDay = previousDate
    ? getEpochDayInTimezone(previousDate, safeTimezone)
    : null;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!latestThought)
        return json(res, 200, { thought: null, has_posted_today: false });

      const timezone = profile.timezone || "UTC";
      const latestDay = getEpochDayInTimezone(
        new Date(latestThought.created_at),
        timezone,
      );
      ...
      return json(res, 200, {
        thought: hasPostedToday ? latestThought : null,
    ...
    if (thought.length > MAX_CONTENT_LENGTH)
      return json(res, 400, {
        error: `thought must be ${MAX_CONTENT_LENGTH} characters or less`,
      });
    ...
    const createdThought = rows?.[0]
      ? {
          ...rows[0],
          text:
            typeof rows[0]?.content?.text === "string"
              ? rows[0].content.text
              : "",
          profile: updatedProfile
            ? {
                ...
                streak_count: Number.isInteger(updatedProfile.streak_count)
                  ? updatedProfile.streak_count
                  : 0,
                is_self: true,
                is_following: false,
              }
