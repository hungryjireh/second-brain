...
export default async function handler(req, res) {
  ...
  try {
    ...
    const targetId = String(
      req.body?.following_id || req.query?.following_id || "",
    ).trim();
    if (!isUuid(targetId))
      return json(res, 400, { error: "following_id must be a valid UUID" });
    if (targetId === userId)
    ...
    if (req.method === "POST") {
      ...
      return json(res, 201, {
        follow: rows?.[0] || { follower_id: userId, following_id: targetId },
      });
    }
