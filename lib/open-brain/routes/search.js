...
export default async function handler(req, res) {
  ...
  try {
    const [userRows, thoughtRows] = await Promise.all([
    ...
    const authorIds = Array.from(
      new Set((thoughtRows || []).map((row) => row.user_id).filter(Boolean)),
    );
    const candidateUserIds = (userRows || [])
      .map((row) => row?.id)
      .filter((id) => Boolean(id) && id !== userId);
    const [authorRows, followRows] = await Promise.all([
      authorIds.length
        ? supabaseRequest("/rest/v1/profiles", {
            method: "GET",
            query: {
      ...
      candidateUserIds.length
        ? supabaseRequest("/rest/v1/follows", {
            ...
            query: {
              ...
              following_id: `in.(${candidateUserIds.join(",")})`,
            },
            authToken: token,
          })
        : Promise.resolve([]),
    ...
    const followingIds = new Set(
      (followRows || []).map((row) => row?.following_id).filter(Boolean),
    );
