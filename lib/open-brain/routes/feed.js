...
async function loadSaveCountByThought({ token, thoughtIds }) {
  ...
  for (let i = 0; i < thoughtIds.length; i += THOUGHT_ID_CHUNK_SIZE) {
    ...
    while (true) {
      const saveRows = await supabaseRequest(
        "/rest/v1/thought_second_brain_saves",
        {
          method: "GET",
          query: {
            select: "thought_id",
      ...
      for (const row of saveRows) {
        ...
        saveCountByThoughtId.set(
          thoughtId,
          Number(saveCountByThoughtId.get(thoughtId) || 0) + 1,
        );
      }
  ...
  for (let i = 0; i < thoughtIds.length; i += THOUGHT_ID_CHUNK_SIZE) {
    ...
    while (true) {
      ...
      for (const row of saveRows) {
        ...
        if (!ownerId) continue;
        saveCountByAuthorId.set(
          ownerId,
          Number(saveCountByAuthorId.get(ownerId) || 0) + 1,
        );
      }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const profiles = allProfileIds.length
      ...
      : [];
    const saveCountByAuthorId = await loadSaveCountByAuthor({
      token,
      authorIds: allProfileIds,
    });
    ...
    const reactionSummary = await loadReactionSummary({
      token,
      thoughtIds: reactionTargets,
      viewerId: userId,
    });
    const saveCountByThoughtId = await loadSaveCountByThought({
      token,
      thoughtIds: reactionTargets,
    });
    ...
    const everyoneWithReactions = appendReactionData(
      everyoneThoughts,
      reactionSummary,
    );
    const followingWithReactions = appendReactionData(
      followingThoughts,
      reactionSummary,
    );
    const everyoneWithSaveCounts = appendThoughtSaveCounts(
      everyoneWithReactions,
      saveCountByThoughtId,
    );
    const followingWithSaveCounts = appendThoughtSaveCounts(
      followingWithReactions,
      saveCountByThoughtId,
    );
    const everyoneWithSaves = appendSecondBrainSaveData(
      everyoneWithSaveCounts,
      savedThoughtIds,
    );
    const followingWithSaves = appendSecondBrainSaveData(
      followingWithSaveCounts,
      savedThoughtIds,
    );
