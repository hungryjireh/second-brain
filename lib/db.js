...
const EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY =
  process.env.EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
const TELEGRAM_TOKEN_ENCRYPTION_KEY = process.env.TELEGRAM_TOKEN_ENCRYPTION_KEY;

if (!EXPO_PUBLIC_SUPABASE_URL) {
  throw new Error(
    "Missing EXPO_PUBLIC_SUPABASE_URL. Add it to .env.local or .env.",
  );
}

if (!EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY) {
  throw new Error(
    "Missing EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY. Add it to .env.local or .env.",
  );
}
...
function getMaskedEnv() {
  return {
    ...
    EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY: mask(
...
function isMaxUserTagsError(err) {
  return /maximum of \d+ tags is allowed per user/i.test(String(err?.message));
}
...
async function supabaseRequest(
  path,
  { method = "GET", query, body, prefer, authToken } = {},
) {
...
export async function migrate() {
  try {
    ...
    console.error(
      "[db] entries(id bigserial pk, user_id text not null, raw_text text, category text, content text, priority int default 0, remind_at bigint null, reminded boolean default false, created_at bigint default extract(epoch from now())::bigint)",
    );
    console.error(
      "[db] settings(user_id text not null, key text not null, value text not null, primary key(user_id,key))",
    );
    logEnvAndError(err);
    throw err;
  }
...
export async function insertEntry({
  ...
  const rows = await supabaseRequest("/rest/v1/entries", {
    ...
    body: [
      {
        user_id: userId,
        description: normalizedDescription || raw_text,
        raw_text,
        category,
        title,
        summary,
        content: normalizedContent || String(summary ?? "").trim() || raw_text,
  ...
  if (entry) {
    try {
      ...
      if (!isMaxUserTagsError(err)) throw err;
      entry.tags = [];
    }
  }
  return entry;
...
export async function getEntry(userId, id, authToken) {
  const rows = await supabaseRequest("/rest/v1/entries", {
    method: "GET",
    query: {
      select: "*",
...
function encodeEntriesCursor(entry) {
  ...
  if (!Number.isInteger(entry.created_at) || !Number.isInteger(entry.id))
    return null;
...
async function getEntriesPage({ userId, authToken, category, limit, cursor }) {
  ...
  const nextCursor = hasMore
    ? encodeEntriesCursor(sliced[sliced.length - 1])
    : null;
...
export async function getEntryCategoryCounts(userId, authToken) {
  const rows = await supabaseRequest("/rest/v1/rpc/get_entry_category_counts", {
    method: "POST",
    body: { p_user_id: String(userId || "").trim() },
    authToken,
  });
  const counts = { reminder: 0, todo: 0, thought: 0, note: 0 };
  for (const row of Array.isArray(rows) ? rows : []) {
    const category = String(row?.category || "").trim();
    if (!Object.prototype.hasOwnProperty.call(counts, category)) continue;
    const total = Number.parseInt(String(row?.total ?? 0), 10);
    counts[category] = Number.isInteger(total) && total > 0 ? total : 0;
  }
  return counts;
}
...
export async function updateEntry(userId, id, updates, authToken) {
  ...
  if (updates.remind_at !== undefined)
    body.remind_at = updates.remind_at ?? null;
...
async function upsertTags(userId, tags, authToken) {
  ...
  const inQuery = deduped
    .map((tag) => quotePostgrestValue(tag.normalized_name))
    .join(",");
  ...
  const existingNormalized = new Set(
    (existingRows || []).map((row) => row.normalized_name),
  );
  const missingTags = deduped.filter(
    (tag) => !existingNormalized.has(tag.normalized_name),
  );
...
async function hydrateEntriesWithTags(userId, entries, authToken) {
  ...
  return entries.map((entry) => ({
    ...entry,
    tags: (tagMap.get(entry.id) ?? []).sort((a, b) =>
...
export async function getTelegramLinkByChatId(chatId) {
  const rows = await supabaseRequest(
    "/rest/v1/rpc/lookup_telegram_link_by_chat_id",
    {
      method: "POST",
...
export async function insertLaunchSignup({
  name,
  email,
  source = "landing-page",
}) {
  try {
    const rows = await supabaseRequest("/rest/v1/launch_signups", {
      method: "POST",
      body: [
        {
          name: String(name || "")
          ...
          email: String(email || "")
            ...
            .toLowerCase()
            .slice(0, 255),
          source: String(source || "landing-page")
            .trim()
            .slice(0, 64),
        },
      ],
    ...
    if (err?.status === 409 || message.includes("duplicate key")) {
      const duplicateError = new Error("Email already signed up");
