...
export async function insertEntry({ userId, raw_text, category, content, remind_at, priority = 0, authToken }) {
  const rows = await supabaseRequest('/rest/v1/entries', {
    method: 'POST',
    body: [{ user_id: userId, raw_text, category, content, remind_at: remind_at ?? null, priority }],
...
export async function deleteEntry(userId, id, authToken) {
  const rows = await supabaseRequest('/rest/v1/entries', {
    method: 'DELETE',
    query: { user_id: `eq.${userId}`, id: `eq.${id}`, select: 'id' },
    prefer: 'return=representation',
    authToken,
  });
...
export async function updateEntry(userId, id, { category, content, remind_at, priority }, authToken) {
  const rows = await supabaseRequest('/rest/v1/entries', {
    method: 'PATCH',
...
export async function getUserTimezone(userId) {
  return (await getSetting(userId, 'timezone')) ?? 'Asia/Singapore';
}
