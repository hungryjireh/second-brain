...
async function getUserTagCount(userId, authToken) {
  ...
  return rows.filter((row) => !isGloballyPermissiveTag(row?.normalized_name))
    .length;
}
...
async function upsertTags(userId, tags, authToken) {
  ...
  const missingBillableTags = missingTags.filter(
    (tag) => !isGloballyPermissiveTag(tag.normalized_name),
  );
