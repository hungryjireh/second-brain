...
const DEFAULT_UNREAL_SPEECH_BASE_URL = "https://api.v8.unrealspeech.com";
...
async function loadAudioBufferFromUri(audioUri, timeoutMs) {
  const uri = String(audioUri || "").trim();
  if (!uri) {
    throw new UnrealSpeechError("audio_uri is required", {
      status: 400,
      code: "unreal_speech_missing_audio_uri",
    });
  }
  ...
  if (!response.ok) {
    ...
    throw new UnrealSpeechError(
      String(
        payload?.error || payload?.message || "Failed to load audio from uri",
      ),
      {
        status: response.status,
        code: "unreal_speech_audio_uri_fetch_failed",
      },
    );
  }
  ...
  if (!audioBuffer || audioBuffer.byteLength === 0) {
    throw new UnrealSpeechError("audio_uri returned empty audio", {
      status: 422,
      code: "unreal_speech_audio_uri_empty",
    });
  }
...
export async function transcribeWithUnrealSpeech({
  ...
  extension = "m4a",
  timeoutMs = DEFAULT_UNREAL_SPEECH_TIMEOUT_MS,
  transcribeFromBufferFn = transcribeFromBuffer,
} = {}) {
  const normalizedAudioUri = String(audioUri || "").trim();
  const normalizedAudio = String(audioBase64 || "").trim();
  if (!normalizedAudioUri && !normalizedAudio) {
    throw new UnrealSpeechError(
      "audio_uri or audio_base64 is required for transcription",
      {
        status: 400,
  ...
  if (!normalizedAudio) {
    if (normalizedAudioUri) {
      const uriAudioBuffer = await loadAudioBufferFromUri(
        normalizedAudioUri,
        timeoutMs,
      );
      const transcript = await transcribeFromBufferFn(
        uriAudioBuffer,
        extension,
      );
      if (!transcript) {
    ...
    throw new UnrealSpeechError("audio_uri or audio_base64 is required", {
      status: 400,
      code: "unreal_speech_missing_audio",
    });
  }
  ...
  const transcript = await transcribeFromBufferFn(audioBuffer, extension);
  if (!transcript) {
    throw new UnrealSpeechError("Transcription failed", {
      status: 422,
      code: "unreal_speech_stt_empty",
