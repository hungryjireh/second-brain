...
async function readRawRequestBuffer(req) {
...
async function handleTranscribeRaw(req, res) {
  try {
    const audioBuffer = await readRawRequestBuffer(req);
    ...
    const transcript = await transcribeWithUnrealSpeech({
      ...
      extension: req.query?.extension || "m4a",
      timeoutMs: Number(req.query?.timeout_ms) || undefined,
    });
    return res.status(200).json({ transcript });
  } catch (error) {
    if (error instanceof UnrealSpeechError) {
...
export default async function handler(req, res) {
  ...
  if (action === "transcribe-raw") {
    return handleTranscribeRaw(req, res);
  }
