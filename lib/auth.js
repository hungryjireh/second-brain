...
export const TELEGRAM_SESSION_VERSION = 2;
...
function ensureSupabaseAuthEnv() {
  if (!EXPO_PUBLIC_SUPABASE_URL) {
    throw new Error(
      "Missing EXPO_PUBLIC_SUPABASE_URL. Add it to environment variables.",
    );
  }
  if (!EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY) {
    throw new Error(
      "Missing EXPO_PUBLIC_SUPABASE_PUBLISHABLE_KEY. Add it to environment variables.",
    );
  }
}
...
export async function refreshSupabaseSession({ refreshToken }) {
  ...
  const authUrl = new URL("/auth/v1/token", EXPO_PUBLIC_SUPABASE_URL);
  authUrl.searchParams.set("grant_type", "refresh_token");
...
export async function verifySupabaseAccessToken(token) {
  ...
  if (!res.ok) {
    const message =
      data?.msg || data?.error_description || data?.error || "unauthorized";
    const err = new Error(message);
    err.status = res.status;
    throw err;
  }
...
export function createTelegramSessionToken(userId, refreshToken) {
  const normalizedRefreshToken = String(refreshToken || "").trim();
  if (!normalizedRefreshToken) {
  ...
  return createAuthJwt({
    sub: userId,
    purpose: TELEGRAM_SESSION_TOKEN_PURPOSE,
    v: TELEGRAM_SESSION_VERSION,
    srt: normalizedRefreshToken,
  });
}

export function createTelegramLinkKey(userId, authToken, refreshToken) {
  const normalizedAuthToken = String(authToken || "").trim();
  if (!normalizedAuthToken) {
    throw new Error("Missing auth token for Telegram link key");
  }
  const normalizedRefreshToken = String(refreshToken || "").trim();
  const authTokenToStore = normalizedRefreshToken
    ? createTelegramSessionToken(userId, normalizedRefreshToken)
    : normalizedAuthToken;
  return createAuthJwt(
    {
      sub: userId,
      purpose: "telegram_link",
      sat: authTokenToStore,
      rat: normalizedAuthToken,
    },
...
export function verifyTelegramLinkKey(token) {
  ...
  const authTokenToStore = typeof payload?.sat === "string" ? payload.sat : "";
  if (!authTokenToStore) {
    throw new Error("Invalid Telegram link key payload");
  }
  const requestAuthToken =
    typeof payload?.rat === "string" ? payload.rat : authTokenToStore;
  if (!requestAuthToken) {
    throw new Error("Invalid Telegram link key payload");
  }
  return { userId, authTokenToStore, requestAuthToken };
}
