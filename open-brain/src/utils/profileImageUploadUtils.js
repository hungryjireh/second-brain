...
export async function pickAndValidateProfileImage() {
  ...
  if (
    Number.isFinite(selectedFileSize) &&
    selectedFileSize > MAX_PROFILE_IMAGE_BYTES
  ) {
...
export async function resolveStorageOwnerSegment({
  token,
  username,
  supabaseConfig,
}) {
  ...
  try {
    ...
    if (authRes.ok) {
      ...
      const userId = String(authPayload?.id || authPayload?.user?.id || "")
        .trim()
...
export async function uploadProfileImageAndGetPublicUrl({
  ...
  const ownerSegment = await resolveStorageOwnerSegment({
    token,
    username,
    supabaseConfig,
  });
  ...
  if (!uploadRes.ok) {
    ...
    if (uploadError.toLowerCase().includes("row-level security")) {
      throw new Error(
        "Upload blocked by storage policy. Please confirm the bucket RLS policy allows uploads to your user-id folder.",
      );
    }
