...
describe("SharedThoughtScreen", () => {
  ...
  it("loads and renders shared thought details", async () => {
    apiRequest.mockResolvedValueOnce({
      thought: {
        text: "A shared idea",
        created_at: "2026-01-02T03:04:00.000Z",
      },
    ...
    fireEvent.changeText(
      getByPlaceholderText("share slug"),
      " my-shared-slug ",
    );
      ...
      expect(apiRequest).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cache: expect.objectContaining({
            ttlMs: CACHE_TTL_MS.SHARED_THOUGHT,
          }),
        }),
      );
  ...
  it("routes to Login when unauthenticated user taps username link", async () => {
    ...
    const { getByText } = render(
      <SharedThoughtScreen
        route={{ params: { slug: "my-shared-slug" } }}
