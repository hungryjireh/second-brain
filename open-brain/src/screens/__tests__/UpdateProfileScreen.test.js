...
describe("UpdateProfileScreen upload", () => {
  ...
  const jwtTokenWithSub =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhYmMxMjMtVVNFUl9pZCJ9.signature";
  ...
  it("shows permission error when media permission is denied", async () => {
    ...
    expect(
      await findByText(
        "Photo library permission is required to upload a profile photo.",
      ),
    ).toBeTruthy();
        ...
        expect.objectContaining({
          ...
          body: expect.objectContaining({
            avatar_url: expect.stringContaining(
              "https://example.supabase.co/storage/v1/object/public/profileImage/user-987/",
            ),
          }),
        }),
      );
  ...
  it("uploads selected image into auth user-id folder when JWT sub is present", async () => {
    ...
    const { getByText, findByText } = render(
      <UpdateProfileScreen token={jwtTokenWithSub} navigation={navigation} />,
    );
    await findByText("Identity");
    ...
    expect(fetchMock.mock.calls[1][0]).toContain(
      "/storage/v1/object/profileImage/abc123-USER_id/",
    );
    expect(fetchMock.mock.calls[1][1].headers.Authorization).toBe(
      `Bearer ${jwtTokenWithSub}`,
    );
  });
  ...
  it("shows an error for unsupported file extension", async () => {
    ...
    expect(
      await findByText(
        "Unsupported image type. Please upload a JPG, PNG, WEBP, GIF, or HEIC image.",
      ),
    ).toBeTruthy();
  ...
  it("shows an error when image exceeds 5MB", async () => {
    ...
    expect(
      await findByText("Image is too large. Maximum allowed size is 5MB."),
    ).toBeTruthy();
    expect(fetchMock).not.toHaveBeenCalled();
  });
