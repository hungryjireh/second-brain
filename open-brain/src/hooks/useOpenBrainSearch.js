...
export function useOpenBrainSearch({
  ...
  const runSearch = useCallback(async (nextQuery = query) => runGuardedOpenBrainSearch({
    query: nextQuery,
    loading,
    setLoading,
    setError,
  ...
  }), [apiRequest, cacheTtlMs, fallbackErrorMessage, loading, query, sortUsersByQuery, token]);
  ...
  const normalizedQuery = useMemo(() => normalizeOpenBrainSearchInput(query), [query]);
